...
function makeParticles(n=38) {
  return Array.from({length:n},(_,i)=>({
    id:i, x:40+Math.random()*20, y:45,
    vx:(Math.random()-0.5)*14, vy:-(Math.random()*10+6),
    color:CONFETTI_COLORS[i%CONFETTI_COLORS.length],
    r:Math.random()*5+3, shape:Math.random()>0.5?'circle':'rect',
    rot:Math.random()*360, rotV:(Math.random()-0.5)*12,
    life:1, decay:Math.random()*0.022+0.02,
  }))
}
...
function ConfettiCanvas({active}) {
  ...
  useEffect(()=>{
    ...
    const tick=()=>{
      ...
      particles.current.forEach(p=>{
        p.x+=p.vx*0.6; p.y+=p.vy*0.6; p.vy+=0.35; p.rot+=p.rotV; p.life-=p.decay
  ...
  return <canvas ref={canvasRef} width={400} height={300} style={{position:'fixed',inset:0,width:'100%',height:'100%',pointerEvents:'none',zIndex:100}}/>
}
  ...
  return (
    <div style={{position:'fixed',inset:0,zIndex:99,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',pointerEvents:'none'}}>
      <div style={{animation:'perfectIn 0.5s cubic-bezier(0.34,1.56,0.64,1) forwards',textAlign:'center'}}>
        ...
        <div style={{fontFamily:"'Playfair Display',serif",fontSize:24,fontWeight:700,color:'#FFD93D',textShadow:'0 0 18px #FFD93D55',letterSpacing:'-0.01em',marginBottom:6}}>Perfect Day</div>
        <div style={{fontSize:10,color:'#888',letterSpacing:'0.12em',textTransform:'uppercase'}}>all habits complete</div>
      </div>
    </div>
  )
}
...
function ReportCard({habits,weekDates,today,onClose}) {
  ...
  return (
    <div style={{position:'fixed',inset:0,zIndex:200,background:'#0f0f13ee',display:'flex',alignItems:'center',justifyContent:'center',backdropFilter:'blur(6px)',animation:'fadeIn 0.2s ease'}} onClick={onClose}>
      <div style={{background:'#18181f',border:'1px solid #2a2a38',borderRadius:20,padding:'32px 28px',width:300,animation:'slideUp 0.35s cubic-bezier(0.34,1.56,0.64,1)'}} onClick={e=>e.stopPropagation()}>
        ...
        <button onClick={onClose} style={{width:'100%',background:'#23232e',border:'1px solid #2a2a38',borderRadius:10,padding:'11px',color:'#888',fontFamily:"'DM Mono',monospace",fontSize:12,cursor:'pointer',letterSpacing:'0.06em'}}>close</button>
      </div>
    </div>
  )
}
...
export default function App() {
  ...
  const prevPctRef=useRef(0), timerRef=useRef(null)
  ...
  useEffect(()=>{
    if(habits.length>0&&todayPct===100&&prevPctRef.current<100){
      setShowPerfect(true); setShowConfetti(true)
  ...
  const S = {
    root:{minHeight:'100vh',background:'#0f0f13',fontFamily:"'DM Mono','Courier New',monospace",display:'flex',flexDirection:'column',alignItems:'center'},
    inner:{width:'100%',maxWidth:480,padding:'30px 16px 52px',boxSizing:'border-box'},
    card:{background:'#18181f',border:'1px solid #21212c',borderRadius:16},
  }
    ...
    <div style={S.root}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        body{background:#0f0f13;-webkit-font-smoothing:antialiased;}
        .hrow{transition:background 0.15s;border-radius:10px;}
        .hrow:hover{background:#1c1c27;}
        .dbtn{width:32px;height:32px;min-width:32px;border-radius:50%;border:none;cursor:pointer;font-family:'DM Mono',monospace;font-size:12px;font-weight:500;transition:transform 0.12s,background 0.18s;display:flex;align-items:center;justify-content:center;flex-shrink:0;}
        .dbtn:active{transform:scale(0.88);}
        .tring{box-shadow:0 0 0 2px #ffffff28;}
        @keyframes checkPop{0%{transform:scale(1)}35%{transform:scale(1.35)}65%{transform:scale(0.88)}100%{transform:scale(1)}}
        @keyframes glowPulse{0%{box-shadow:0 0 0 0 rgba(255,255,255,0.35)}55%{box-shadow:0 0 0 8px rgba(255,255,255,0)}100%{box-shadow:0 0 0 0 rgba(255,255,255,0)}}
        .jdone{animation:checkPop 0.42s cubic-bezier(0.34,1.56,0.64,1),glowPulse 0.6s ease forwards;}
        .ptrack{background:#1c1c26;border-radius:99px;height:3px;width:100%;overflow:hidden;}
        .pfill{height:100%;border-radius:99px;transition:width 0.6s cubic-bezier(0.34,1.56,0.64,1);}
        .mtrack{background:#1e1e28;border-radius:99px;height:3px;width:44px;flex-shrink:0;}
        .mfill{border-radius:99px;height:3px;transition:width 0.35s ease;}
        .abtn{background:#18181f;border:1px dashed #2b2b38;color:#484858;border-radius:12px;padding:13px;width:100%;cursor:pointer;font-family:'DM Mono',monospace;font-size:12px;transition:all 0.15s;letter-spacing:0.06em;}
        .abtn:hover{border-color:#555;color:#aaa;background:#1c1c27;}
        .ifield{background:#0f0f13;border:1px solid #21212c;border-radius:10px;color:#eee;font-family:'DM Mono',monospace;font-size:13px;padding:10px 14px;outline:none;width:100%;transition:border-color 0.15s;}
        .ifield:focus{border-color:#404050;}
        .ifield::placeholder{color:#32323f;}
        .cfbtn{border:none;border-radius:10px;padding:10px 20px;font-family:'DM Mono',monospace;font-size:12px;font-weight:500;cursor:pointer;transition:all 0.15s;letter-spacing:0.04em;}
        .cfbtn:hover{opacity:0.85;transform:translateY(-1px);}
        .ccbtn{background:transparent;border:1px solid #272732;border-radius:10px;padding:10px 16px;font-family:'DM Mono',monospace;font-size:12px;color:#555;cursor:pointer;transition:all 0.15s;}
        .ccbtn:hover{border-color:#555;color:#aaa;}
        .csw{width:20px;height:20px;border-radius:50%;cursor:pointer;border:2px solid transparent;transition:transform 0.12s;flex-shrink:0;}
        .csw:hover{transform:scale(1.2);}
        .csw.sel{border-color:#fff;}
        .delbtn{background:transparent;border:none;color:#2e2e3e;cursor:pointer;font-size:15px;padding:4px 6px;border-radius:6px;transition:color 0.15s,background 0.15s;line-height:1;flex-shrink:0;}
        .delbtn:hover{color:#ff6b6b;background:#ff6b6b14;}
        .ibtn{background:transparent;border:none;cursor:pointer;font-size:14px;opacity:0.35;transition:opacity 0.15s;padding:4px;border-radius:6px;}
        .ibtn:hover{opacity:0.8;}
        .sbadge{font-size:9px;border-radius:99px;padding:1px 6px;letter-spacing:0.05em;transition:all 0.2s;}
        @keyframes slideIn{from{opacity:0;transform:translateY(6px)}to{opacity:1;transform:translateY(0)}}
        .sin{animation:slideIn 0.2s ease;}
        @keyframes perfectIn{0%{opacity:0;transform:scale(0.7) translateY(10px)}100%{opacity:1;transform:scale(1) translateY(0)}}
        @keyframes fadeIn{from{opacity:0}to{opacity:1}}
        @keyframes slideUp{from{opacity:0;transform:translateY(24px)}to{opacity:1;transform:translateY(0)}}
        .mcopy{font-size:10px;letter-spacing:0.1em;text-transform:uppercase;transition:color 0.4s;}
        .crown{font-size:10px;position:absolute;top:-12px;left:50%;transform:translateX(-50%);filter:drop-shadow(0 0 4px #FFD93Daa);}
      `}</style>
      ...
      <div style={S.inner}>
        <div style={{marginBottom:24}}>
          <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:4}}>
            ...
            <div style={{display:'flex',gap:4}}>
              <button className="ibtn" onClick={()=>setSoundOn(s=>!s)}>{soundOn?'🔔':'🔕'}</button>
              <button className="ibtn" onClick={()=>setShowReport(true)}>📊</button>
            </div>
          </div>

          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16}}>
            <h1 style={{fontFamily:"'Playfair Display',serif",color:'#e2e2ea',fontSize:28,fontWeight:700,letterSpacing:'-0.01em'}}>Habits</h1>
          ...
          <div className="mcopy" style={{color:todayPct===100?'#FFD93D':todayPct>=50?'#6BCB77':'#38384a',marginBottom:14}}>{microCopy}</div>

          <div style={{marginBottom:18}}>
            <div style={{display:'flex',justifyContent:'space-between',marginBottom:7}}>
              <span style={{fontSize:9,color:'#38384a',letterSpacing:'0.12em',textTransform:'uppercase'}}>this week</span>
              <span style={{fontSize:9,color:weeklyPct>=80?'#6BCB77':'#48485a',letterSpacing:'0.06em',transition:'color 0.3s'}}>{doneWk}/{possibleWk} · {weeklyPct}%</span>
            </div>
            <div className="ptrack">
              <div className="pfill" style={{width:`${weeklyPct}%`,background:weeklyPct===100?'linear-gradient(90deg,#FFD93D,#FF9A3C)':weeklyPct>=60?'linear-gradient(90deg,#4D96FF,#6BCB77)':'#4D96FF'}}/>
            </div>
          </div>
            ...
            {weekDates.map((date,i)=>{
              ...
              return (
                <div key={date} style={{flex:1,textAlign:'center',position:'relative'}}>
                  ...
                  <div style={{fontSize:9,marginBottom:perfect?14:5,letterSpacing:'0.04em',color:isToday?'#888':'#2e2e3e',fontWeight:isToday?500:400}}>{DAYS[i]}</div>
        ...
        <div style={{display:'flex',alignItems:'center',marginBottom:5,paddingRight:28}}>
          ...
          <div style={{display:'flex',gap:4}}>
            {weekDates.map((date,i)=>(
              <div key={date} style={{width:32,textAlign:'center',fontSize:9,color:date===today?'#666':'#272736',letterSpacing:'0.04em'}}>{DAYS[i]}</div>
            ))}
          </div>
        </div>

        <div style={{...S.card,padding:'6px',marginBottom:10}}>
          {habits.length===0&&<div style={{textAlign:'center',color:'#2e2e3e',padding:'30px 0',fontSize:12,letterSpacing:'0.06em'}}>No habits yet</div>}
          {habits.map((hab,idx)=>{
            const streak=computeStreak(hab.completions), best=hab.bestStreak||0, wk=weekCount(hab)
            return (
              <div key={hab.id} className="hrow sin" style={{display:'flex',alignItems:'center',padding:'10px 8px',gap:6,borderBottom:idx<habits.length-1?'1px solid #16161e':'none'}}>
                <div style={{flex:1,minWidth:0,marginRight:4}}>
                  ...
                  <div style={{display:'flex',alignItems:'center',gap:7,flexWrap:'wrap'}}>
                    <div className="mtrack"><div className="mfill" style={{width:`${(wk/7)*100}%`,background:hab.color}}/></div>
                    <span style={{fontSize:9,color:'#38384a',letterSpacing:'0.07em'}}>{wk}/7</span>
                    {streak>=2&&<span className="sbadge" style={{color:streak>=7?'#FFD93D':'#666',background:streak>=7?'#FFD93D15':'#1e1e28',border:`1px solid ${streak>=7?'#FFD93D35':'#28283a'}`}}>{streak>=7?'🔥':'⚡'} {streak}d</span>}
                    {best>=3&&<span style={{fontSize:9,color:'#383848',letterSpacing:'0.05em'}}>best {best}d</span>}
                  </div>
                </div>

                <div style={{display:'flex',gap:3}}>
                  {weekDates.map(date=>{
                    const done=!!hab.completions[date], isToday=date===today, isFuture=date>today
                    const key=`${hab.id}-${date}`
                    return (
                      <button key={date}
                        className={['dbtn',isToday?'tring':'',justDone===key?'jdone':''].filter(Boolean).join(' ')}
                ...
                {deletingId===hab.id?(
                  <div style={{display:'flex',gap:3,alignItems:'center',flexShrink:0}}>
                    ...
                    <button onClick={()=>setDeletingId(null)} className="ccbtn" style={{padding:'3px 7px',fontSize:10}}>✕</button>
                  </div>
                ):(
                  <button className="delbtn" onClick={()=>setDeletingId(hab.id)}>×</button>
                )}
              </div>
            )
          })}
        ...
        {adding?(
          <div style={{...S.card,padding:14,marginBottom:10}} className="sin">
            <input className="ifield" placeholder="Habit name..." value={newName}
            ...
            <div style={{display:'flex',gap:7,alignItems:'center',marginBottom:14}}>
              ...
              {COLORS.map(c=><button key={c} className={`csw${newColor===c?' sel':''}`} style={{background:c}} onClick={()=>setNewColor(c)}/>)}
            </div>
            <div style={{display:'flex',gap:8}}>
              <button className="cfbtn" style={{background:newColor,color:'rgba(0,0,0,0.8)',flex:1}} onClick={addHabit}>Add habit</button>
              <button className="ccbtn" onClick={()=>setAdding(false)}>Cancel</button>
            </div>
          </div>
        ):(
          <button className="abtn" onClick={()=>setAdding(true)}>+ new habit</button>
        )}

        <div style={{marginTop:18,textAlign:'center',fontSize:9,color:'#1e1e28',letterSpacing:'0.12em',textTransform:'uppercase'}}>tap a day to toggle · saved locally</div>
      </div>
    </div>
  )
}
